...
function pauseUnpause(pause) {
    paused = pause;
    runPauseButton.label = paused ? "Futtatás" : "Megállitás";
}
...
function setup() {
    ...
    runPauseButton = new Button("Futtatás", 430, 20, 70, 30, runpause);
    uiElements.push(runPauseButton);
	uiElements.push(new Button("Újra", 430, 100, 70, 30, restart));
    uiElements.push(new Button("Következő", 430, 60, 70, 30, restart));
...
function searchStep() {
    if (!paused || stepsAllowed > 0) {
        ...
        switch (result) {
            case -1:
                status = "Nincs megoldás";
                logTimings();
                pauseUnpause(true);
                break;
            case 1:
                status = "Elérte a célt!";
                logTimings();
                pauseUnpause(true);
                break;
            case 0:
...
function draw() {
    ...
    text("Keresés Állapota - " + status, 10, 450);
